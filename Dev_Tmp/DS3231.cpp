...
void DS3231::startClock(void)        // set the ClockHalt bit low to start the rtc
{
...
void DS3231::stopClock(void)         // set the ClockHalt bit high to stop the rtc
{
  Wire.beginTransmission(DS3231_I2C_ADDRESS);
  Wire.write((uint8_t)0x00);                      // Register 0x00 holds the oscillator start/stop bit
  Wire.endTransmission();
...
void DS3231::fillByHMS(uint8_t _hour, uint8_t _minute, uint8_t _second)
{
    // assign variables
    hour = _hour;
    minute = _minute;
...
void DS3231::fillByYMD(uint16_t _year, uint8_t _month, uint8_t _day)
{
    year = _year-2000;
    month = _month;
    dayOfMonth = _day;
}
void DS3231::fillDayOfWeek(uint8_t _dow)
{
    dayOfWeek = _dow;
}
